...
sudo apt-get update
sudo apt-get install -y libclang-dev linux-perf shellcheck zstd
...
# Install groonga
DEB_PKG="apache-arrow-apt-source-latest-$(lsb_release --codename --short).deb"
curl -sSLO --output-dir /tmp "https://apache.jfrog.io/artifactory/arrow/$(lsb_release --id --short | tr '[:upper:]' '[:lower:]')/${DEB_PKG}"
sudo apt-get install -y /tmp/"${DEB_PKG}"
sudo apt-get update -y

DEB_PKG="groonga-apt-source-latest-$(lsb_release --codename --short).deb"
curl -sSLO --output-dir /tmp "https://packages.groonga.org/debian/${DEB_PKG}"
sudo apt-get install -y /tmp/"${DEB_PKG}"
sudo apt-get update -y
